...
#if canImport(user_domainFFI)
import user_domainFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_user_domain_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_user_domain_rustbuffer_free(self, $0) }
    }
}
...
private func makeRustCall<T, E: Swift.Error>(
    ...
    uniffiEnsureUserDomainInitialized()
 ...
 */
public protocol GetUsersUseCaseImplProtocol: AnyObject, Sendable {
...
@_documentation(visibility: private)
#endif
public struct FfiConverterTypeUserDomainModel: FfiConverterRustBuffer {
    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> UserDomainModel {
        return
            try UserDomainModel(
    ...
    public static func write(_ value: UserDomainModel, into buf: inout [UInt8]) {
...
@_documentation(visibility: private)
#endif
public func FfiConverterTypeUserDomainModel_lift(_ buf: RustBuffer) throws -> UserDomainModel {
    return try FfiConverterTypeUserDomainModel.lift(buf)
}
...
@_documentation(visibility: private)
#endif
fileprivate struct FfiConverterSequenceTypeUserDomainModel: FfiConverterRustBuffer {
    typealias SwiftType = [UserDomainModel]

    public static func write(_ value: [UserDomainModel], into buf: inout [UInt8]) {
        let len = Int32(value.count)
        writeInt(&buf, len)
        for item in value {
            FfiConverterTypeUserDomainModel.write(item, into: &buf)
        }
    }

    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> [UserDomainModel] {
        let len: Int32 = try readInt(&buf)
        var seq = [UserDomainModel]()
        seq.reserveCapacity(Int(len))
        for _ in 0 ..< len {
            seq.append(try FfiConverterTypeUserDomainModel.read(from: &buf))
        }
        return seq
    }
}
...
public func uniffiEnsureUserDomainInitialized() {
